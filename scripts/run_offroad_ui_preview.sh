...
SESSION_NAME="${SNITH_UI_SESSION:-snithpilot-ui-offroad}"
DISPLAY_NUM="${DISPLAY_NUM:-99}"
NOVNC_PORT="${NOVNC_PORT:-6080}"
VNC_PORT="${VNC_PORT:-5901}"
LOG_PATH="${LOG_PATH:-/tmp/snithpilot-ui-offroad.log}"
...
tmux new-session -d -s "${SESSION_NAME}" \
  "cd '${ROOT_DIR}' && DISPLAY_NUM='${DISPLAY_NUM}' NOVNC_PORT='${NOVNC_PORT}' VNC_PORT='${VNC_PORT}' FPS='${FPS:-15}' ./scripts/run_onroad_browser.sh --nav --offroad --demo >'${LOG_PATH}' 2>&1"

sleep 2

echo "Started offroad UI preview."
