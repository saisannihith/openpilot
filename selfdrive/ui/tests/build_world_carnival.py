...
def primitive_color(document, primitive):
  ...
  if name == '2022_Kia_Carnival_Panthera_Metal':
    return np.array((245, 247, 250), dtype=np.uint8)
  if name in ('2022_Kia_Carnival_BrakeLightsMain', '2022_Kia_Carnival_DarkRed',
              '2022_Kia_Carnival_Reflector', '2022_Kia_Carnival_ReflectorGlass'):
    return np.array((205, 28, 34), dtype=np.uint8)
  if any(token in name for token in ('Windows', 'Interior', 'Tires', 'UnderCarrier', 'Grille', 'Black')):
    return np.array((24, 29, 35), dtype=np.uint8)
  if any(token in name for token in ('HeadLights', 'DRLs', 'Chrome', 'Mirrors', 'Rims', 'BrakeRotors')):
    return np.array((190, 199, 209), dtype=np.uint8)
  factor = material.get('pbrMetallicRoughness', {}).get('baseColorFactor', (1., 1., 1., 1.))
  return np.rint(np.clip(factor[:3], 0., 1.) * 255).astype(np.uint8)
...
def load_triangles(document, binary):
  ...
  for node, world in nodes(document):
    ...
    for primitive in document['meshes'][node['mesh']]['primitives']:
      ...
      colors.append(np.tile(primitive_color(document, primitive), (len(indices), 1)))
  assert vertices
  return np.concatenate(vertices), np.concatenate(normals), np.concatenate(colors)


def orient_and_scale(vertices, normals, colors):
  ...
  return oriented.astype(np.float32), oriented_normals.astype(np.float32), colors, longitudinal, rear_sign


def bake_lighting(normals, colors):
  # The source GLB carries smooth authored normals. Bake a restrained studio
  # light from those normals so its curved panels remain readable without a
  # runtime shader or any geometry reduction.
  light = np.array((-.32, .82, .47), dtype=np.float32)
  light /= np.linalg.norm(light)
  lighting = .56 + .36 * np.clip(normals @ light, 0, 1) + .08 * np.clip(normals[:, 1], 0, 1)
  rgb = np.clip(colors * lighting[:, None], 0, 255)
  return np.column_stack((rgb, np.full(len(rgb), 255))).astype(np.uint8)
...
def main():
  ...
  vertices, normals, colors = load_triangles(document, binary)
  vertices, normals, colors, axis, rear_sign = orient_and_scale(vertices, normals, colors)
  rgba = bake_lighting(normals, colors)
  assert len(vertices) % 3 == 0 and 0 < len(vertices) <= 1_500_000 and np.isfinite(vertices).all()
  args.output.parent.mkdir(parents=True, exist_ok=True)
  np.savez_compressed(args.output, vertices=vertices, colors=rgba)
